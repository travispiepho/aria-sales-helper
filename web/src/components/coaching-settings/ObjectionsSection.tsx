...
// ObjectionsSection — extracted from the standalone ObjectionsPage.tsx
// (2026-08-18, Troy Hacker's "Rebuttal list to objections" request) into
// the merged Coaching Settings page (2026-08-30,
// aria_coaching_settings_merge_objections_frontend), per Gabe's ask to
// combine the Objections tab and the Coaching tab into one central
// "coaching settings" home. Logic/behavior is UNCHANGED from the original
// page — only the page-chrome (AppPageLayout/header/back-nav) was
// stripped out since this now renders as a section inside
// CoachingSettingsPage.tsx's own single AppPageLayout, selected via its
// segmented sub-nav. Still a shared (not admin-gated) reference library —
// see server.js's route-block comment for the auth-model rationale.
//
// Two-pane-on-one-screen pattern (list view <-> detail view, toggled by
// local state rather than a nested route) is preserved as-is.
...
export default function ObjectionsSection() {
  ...
  return view.mode === 'list' ? (
    <ObjectionsList onOpen={(id) => setView({ mode: 'detail', id })} />
  ) : (
...
function ObjectionsList({ onOpen }: { onOpen: (id: string) => void }) {
  ...
  return (
    <div data-coaching-settings-section="objections">
      <p className="text-sm text-gray-500 mb-4">Browse common customer objections and the rebuttals that work.</p>
...
function ObjectionDetail({ id, onBack }: { id: string; onBack: () => void }) {
  ...
  return (
    <div data-coaching-settings-section="objection-detail">
      <button
        onClick={onBack}
        className="text-sm font-medium text-brand-700 hover:text-brand-800 mb-4 inline-flex items-center gap-1"
      >
        ← Back to Objections
      </button>
