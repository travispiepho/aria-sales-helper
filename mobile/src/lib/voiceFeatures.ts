/**
 * voiceFeatures.ts (mobile) — Voice feature extraction from raw PCM samples.
 *
 * Ported verbatim from app/web/src/lib/voiceFeatures.ts (2026-08-10, mobile
 * voice-recognition port task). The algorithm has ZERO browser-specific
 * dependencies (no `window`, `AudioContext`, DOM types, etc.) — it only
 * operates on `Float32Array`/`Float64Array` PCM sample arrays, which is
 * exactly what this file's caller (`voiceEnrollment.ts`) produces after
 * decoding a WAV file's PCM data on-device. Because the input contract is
 * identical (16kHz mono, normalized -1..1 float samples — see
 * `voiceEnrollment.ts`'s `int16PcmToFloat32()` for how the on-device WAV
 * bytes are normalized to match this), this is a byte-for-byte port with NO
 * algorithmic changes: the mobile app computes the exact same feature
 * vector shape the backend already stores/compares for web-enrolled voice
 * prints (see app/server/voiceFeatures.js, the backend's own copy of this
 * same algorithm, used for scoring during live meetings).
 *
 * Do not fork the algorithm here — if the web version changes, port the
 * change here too so mobile-enrolled and web-enrolled voice prints stay
 * comparable against the same backend scoring logic.
 */
