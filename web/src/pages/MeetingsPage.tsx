...
export default function MeetingsPage() {
  ...
  async function handleDownload(event: React.MouseEvent, meeting: Meeting) {
    event.stopPropagation();
    setDownloadingId(meeting.id);
    try {
      const [full, { segments }] = await Promise.all([
        getMeeting(meeting.id),
        getMeetingSegments(meeting.id),
      ]);
      ...
      if (full.summary) lines.push('SUMMARY', '─'.repeat(60), full.summary, '');
      lines.push('TRANSCRIPT', '─'.repeat(60));
      if (segments.length === 0) {
        lines.push('(no transcript recorded)');
      } else {
        let lastLabel = '';
        segments.forEach(segment => {
          const label = labels[segment.speaker] || segment.speaker;
          if (label !== lastLabel) {
      ...
      lines.push('', '='.repeat(60), 'Generated by ARIA — CertaPro Grand Haven');
      const url = URL.createObjectURL(new Blob([lines.join('\n')], { type: 'text/plain' }));
      const anchor = document.createElement('a');
      anchor.href = url;
      anchor.download = `${displayTitle.replace(/[^a-z0-9]/gi, '-').toLowerCase()}-transcript.txt`;
      anchor.click();
          ...
          return (
            <article key={meeting.id} className="relative group">
              ...
              <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1">
                <button
                  onClick={event => handleDownload(event, meeting)}
                  disabled={downloadingId === meeting.id}
                  className="w-11 h-11 text-gray-400 hover:text-blue-500 transition-colors rounded-lg hover:bg-blue-50 disabled:opacity-40"
                  aria-label={`Download transcript for ${meeting.title || meeting.customer_name || 'meeting'}`}
                  title="Download transcript"
                >
                  {downloadingId === meeting.id ? '…' : '⬇️'}
                </button>
                <button
